...
panelWidget["progressH"] = {
	name = "progressH",
	realName = "Horizontal Progress Bar",
	wireType = 1,
	...
	modDraw =		function (ent, widget)
						...
						if (widget.direction > 0) then
							x1 = ((widget.X + ((widget.W - 6) * widget.cvalue) + 3) * ent.XdrawCoef) + ent.XdrawOffs
							w = (widget.W - ((widget.W - 6) * widget.cvalue)) * ent.XdrawCoef
						else
							x1 = ((widget.X +  3) * ent.XdrawCoef) + ent.XdrawOffs
							w = ((widget.W - 6) * widget.cvalue) * ent.XdrawCoef
						end
						local y1 = ((widget.Y + 3) * ent.YdrawCoef) + ent.YdrawOffs
						local h = (widget.H - 6) * ent.YdrawCoef
						surface.DrawRect(x1, y1, w, h)
					end,
