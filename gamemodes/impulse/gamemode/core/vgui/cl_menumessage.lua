...
function PANEL:SetMessage(uid)
	...
	self.desc:SetFont("Impulse-Elements17-Shadow")
	...
	if self.Message.url then
		...
		self.url:SetFont("Impulse-Elements17-Shadow")
...
function PANEL:Paint(w,h)
	...
	if self.Message then
		...
		draw.SimpleText(self.Message.title, "Impulse-Elements22-Shadow", 10, 16, nil, nil, TEXT_ALIGN_CENTER)
